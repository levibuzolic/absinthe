...
test("KNOWN CLIENT LIMITATION: both engines reproduce Apollo losing a stream inside a streamed item", async (t) => {
  ...
  for (const backend of [server, reference]) {
    ...
    // Apollo initializes streamPositions from initial data and deferred data,
    // but not from streamed items. Both inner items arrive over HTTP and all IDs
    // complete, yet Apollo leaves the nested list empty. This assertion records
    // the exact pinned-client limitation, NOT successful reconstruction.
    assert.deepEqual(data(final), {
...
test("Apollo cancellation closes sockets and workers; known reader rejection is isolated and asserted", async () => {
